...
public class PingPangPosition : MonoBehaviour {
	...
	void Update () {
		if(axisBy == AxisBy.x)
        {
            ...
            else if(tr.position.x <= originalPos.x - moveDis)
            {
                tr.position = originalPos;
            }
        }else if(axisBy == AxisBy.y)
        {
            tr.position = tr.position - new Vector3(0, 1, 0) * moveSpeed;
            if (tr.position.y <= originalPos.y - moveDis)
            {
                tr.position = originalPos;
            }
